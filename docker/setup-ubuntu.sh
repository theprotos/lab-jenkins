...
apt-get -y -qq install ca-certificates curl gnupg lsb-release jq && printf "\n\tpackages [ OK ]"
...
curl -sL $composeURL -o /usr/bin/docker-compose && chmod +x /usr/bin/docker-compose && printf "\tdocker-compose [ OK ]"
...
printf "\n  [$(date +'%Y-%m-%dT%H:%M:%S%z')]: Start cluster"
cd /vagrant/cluster
chmod +x *.sh
sh 00-generate-keys.sh
