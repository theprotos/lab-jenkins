...
echo -e "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: Add jenkins LTS repo"
...
echo -e "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: Set Jenkins variables"
...
echo -e "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: Copy Jenkins files"
cp plugins.txt $JENKINS_HOME/
mkdir -p $CASC_JENKINS_CONFIG && cp jcasc.yml $CASC_JENKINS_CONFIG/

echo -e "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: Install plugins via jenkins-plugin-manager \n$pimt_url"
...
echo -e "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: Start and Setup Jenkins daemon"
